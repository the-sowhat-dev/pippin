...
export function VideoStrategy({ videoUrl, thumbnailUrl }: VideoStrategyProps) {
  ...
  return (
    <div className="container mx-auto py-6 lg:py-8 max-w-3xl">
      <div className="relative w-full mx-auto" style={{ aspectRatio: "16/9" }}>
        {!isVideoLoaded ? (
          // Facade: Static thumbnail with play button
          <button
            onClick={handlePlayClick}
            className="relative w-full h-full group cursor-pointer overflow-hidden rounded-lg ring-1 ring-white/40 focus:outline-none focus:ring-4 focus:ring-blue-500 transition-all"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-red-600/90 rounded-2xl p-3 md:px-6 md:py-3 group-hover:scale-110 group-hover:bg-red-700 transition-all duration-300 shadow-lg">
                <PlayIcon className="text-white w-6 h-6 md:w-12 md:h-12" />
              </div>
            </div>
