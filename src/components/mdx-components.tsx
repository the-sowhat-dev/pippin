...
const components = {
  ...
  a: ({ href, children, ...props }: AnchorProps) => {
    const className = "text-green-600 hover:text-green-500 font-semibold";
    if (href?.startsWith("/")) {
      return (
    ...
    <table className="w-full border-collapse mb-8">
      <thead>
        <tr>
          {data.headers.map((header, index) => (
            <th key={index} className="border border-green-200 bg-green-100 p-3 text-left">
              {header}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {data.rows.map((row, index) => (
          <tr key={index} className={index % 2 === 0 ? "bg-green-50" : "bg-yellow-50"}>
            {row.map((cell, cellIndex) => (
              <td key={cellIndex} className="border border-green-300 p-3">
                {cell}
              </td>
            ))}
          </tr>
  ...
  blockquote: (props: BlockquoteProps) => (
    <blockquote
      className="ml-[0.075em] border-l-3 border-green-300 pl-4 !text-green-500"
      {...props}
    />
  ),
