...
const Strategy: React.FC<StrategyProps> = ({ imagePath, title, text, canva, brand, maximize }) => {
  ...
  return (
    <section className={`p-16 sm:p-24 min-h-[600px] ${gradientBackgroundColor}`}>
      <h1 className={`${mulish.className} text-2xl sm:text-4xl text-center flex justify-center pb-12`}>
        {title}
        {brand && <span className="text-sm">®</span>}
      </h1>

      <div className={`flex flex-col justify-between sm:grid sm:grid-cols-2 gap-4`}>
        <div className={`flex items-center flex-col justify-center text-xl sm:text-2xl ${imageAlignment} text-center`}>
          <div>{text}</div>
        </div>

        {!maximize ? (
          <div className="flex items-center justify-center my-11">
            <Image src={imagePath} alt="Strategy Image" width={280} height={200} style={{ objectFit: 'contain' }} />
          </div>
        ) : (
